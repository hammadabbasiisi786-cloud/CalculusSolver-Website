...
import { Card, Code, Grid, PageWrap, Divider, SectionTitle } from "../components/ui.jsx";

const LAYERS = [
  {
    id: 0, icon: "⟨⟩", label: "Tokenizer", color: "#06B6D4",
...
// "END"        → 2`,
  ...
  {
    id: 1, icon: "🔷", label: "Tree Encoder", color: "#A855F7",
...
class TreeEncoder(nn.Module):
  ...
  {
    id: 2, icon: "📋", label: "Rule Head", color: "#F59E0B",
...
class RuleHead(nn.Module):
  ...
  {
    id: 3, icon: "🔁", label: "Tree Decoder", color: "#EF4444",
...
def decode_step(prev_tokens, encoder_out,
  ...
  {
    id: 4, icon: "💬", label: "Step Tracer", color: "#10B981",
...
const TRAINING_STAGES = [
  {
    n: 1, label: "Masked SLaNg Tree Pretraining", color: "#6366F1",
...
# Output: checkpoints/pretrain/best.pt`,
  ...
  {
    n: 2, label: "Supervised Fine-Tuning", color: "#A855F7",
...
# Output: checkpoints/sft/best.pt`,
  ...
  {
    n: 3, label: "SLaNg-in-the-Loop Hard Example Training", color: "#EF4444",
...
const DATA_SOURCES = [
  { name: "Synthetic (generate_synthetic.js)", pairs: "5,000,000", color: "#10B981", desc: "Random SLaNg trees + slangmath solves them. Biased toward rare ops (Lagrange, series)." },
  { name: "AP Calculus AB/BC", pairs: "40K / 65K", color: "#6366F1", desc: "38% rejection rate from latex_to_slang.js parse failures." },
  { name: "MIT 18.01 (Single Variable)", pairs: "120K / 180K", color: "#A855F7", desc: "33% rejection rate." },
  { name: "MIT 18.02 (Multivariable)", pairs: "200K / 280K", color: "#06B6D4", desc: "29% rejection rate." },
  { name: "Taylor Series Expansion", pairs: "80K / 110K", color: "#F59E0B", desc: "27% rejection rate." },
];
...
export default function CalculusSolverPage() {
  ...
  return (
    <PageWrap>
      ...
      {/* Model architecture */}
      <SectionTitle sub="Click each component to explore.">Model Architecture (4 Components)</SectionTitle>
      <Grid cols={2} gap={16} style={{ marginBottom: 32 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {LAYERS.map((l, i) => (
            <button key={l.id} onClick={() => setActiveLayer(i)} style={{
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: i === activeLayer ? l.color : "#94A3B8" }}>{l.label}</div>
                  <div style={{ fontSize: 10, color: "#475569", marginTop: 2 }}>{l.sub}</div>
                </div>
              </div>
            </button>
          ))}
          ...
          <Card style={{ marginTop: 8, padding: "14px 16px" }}>
            <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.1em", marginBottom: 8 }}>COMBINED TRAINING LOSS</div>
...
+ step_tracer  × 0.5   // descriptions`}</Code>
        ...
        <Card accent={layer.color + "40"}>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 11, color: layer.color, letterSpacing: "0.1em", marginBottom: 4 }}>{layer.sub}</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: "#F1F5F9", marginBottom: 6, display: "flex", gap: 10, alignItems: "center" }}>
              <span>{layer.icon}</span> {layer.label}
            </div>
            <p style={{ margin: 0, fontSize: 12.5, color: "#94A3B8", lineHeight: 1.65 }}>{layer.desc}</p>
          </div>
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 10, color: "#475569", letterSpacing: "0.1em", marginBottom: 8 }}>KEY DETAILS</div>
            {layer.detail.map((d, i) => (
              <div key={i} style={{ display: "flex", gap: 8, padding: "6px 0", borderBottom: "1px solid #0F172A", fontSize: 12, color: "#94A3B8" }}>
      ...
      {/* Training stages */}
      <SectionTitle sub="Three sequential stages — each checkpoint is the starting point for the next.">Training Pipeline</SectionTitle>
        ...
        {TRAINING_STAGES.map((s, i) => (
          <div key={s.n} style={{ display: "flex", alignItems: "center" }}>
            <button onClick={() => setActiveStage(i)} style={{
              ...
              borderBottom: i === activeStage ? `2px solid ${s.color}` : "2px solid transparent",
              color: i === activeStage ? s.color : "#475569", cursor: "pointer", fontSize: 12,
            }}>
              Stage {s.n}: {s.label}
            </button>
      ...
      <Card accent={stage.color + "40"} style={{ marginBottom: 32 }}>
        <Grid cols={2} gap={20}>
          <div>
            <div style={{ fontSize: 11, color: stage.color, letterSpacing: "0.1em", marginBottom: 4 }}>{stage.script}</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "#F1F5F9", marginBottom: 10 }}>Stage {stage.n}: {stage.label}</div>
            <p style={{ margin: 0, fontSize: 13, color: "#94A3B8", lineHeight: 1.65 }}>{stage.desc}</p>
          </div>
      ...
      {/* Data pipeline */}
      <SectionTitle sub="5M+ training pairs, all verified by slangmath before use.">Data Pipeline</SectionTitle>
      <Grid cols={1} gap={10}>
        {DATA_SOURCES.map(d => (
          <div key={d.name} style={{
            ...
            <div style={{ minWidth: 110, fontSize: 14, fontWeight: 700, color: d.color }}>{d.pairs}</div>
            <div>
              <div style={{ fontSize: 12, color: "#F1F5F9", fontWeight: 600, marginBottom: 3 }}>{d.name}</div>
      ...
      {/* Inference */}
      <SectionTitle sub="Browser-ready JS class + FastAPI backend.">Inference Stack</SectionTitle>
      <Grid cols={2} gap={16}>
        <Card>
          <div style={{ fontSize: 11, color: "#06B6D4", letterSpacing: "0.1em", marginBottom: 10 }}>JS CLIENT (inference/CalculusSolver.js)</div>
...
// verified by slangmath post-decode`}</Code>
        ...
        <Card>
          <div style={{ fontSize: 11, color: "#A855F7", letterSpacing: "0.1em", marginBottom: 10 }}>FASTAPI BACKEND (api/app.py)</div>
          <Code>{`# Start server
uvicorn api.app:app \\
  --host 0.0.0.0 \\
  --port 8000 \\
...
# - verifier.js checks final answer`}</Code>
      ...
      {/* Evaluation */}
      <SectionTitle sub="Three benchmark sets, all measured by numerical equivalence.">Evaluation</SectionTitle>
      <Grid cols={3} gap={12}>
        {[
          { name: "AP Calculus AB/BC", file: "ap_calculus.json", color: "#6366F1", note: "Standard US high-school calculus problems" },
          { name: "MIT 18.01 / 18.02", file: "mit_ocw.json", color: "#A855F7", note: "Single + multivariable university calculus" },
          { name: "Multivariable", file: "multivariable.json", color: "#06B6D4", note: "Gradient, Lagrange, surface integrals" },
        ].map(b => (
          <Card key={b.name} accent={b.color + "30"}>
            <div style={{ fontSize: 11, color: b.color, letterSpacing: "0.1em", marginBottom: 6 }}>{b.file}</div>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#F1F5F9", marginBottom: 6 }}>{b.name}</div>
            <div style={{ fontSize: 12, color: "#64748B", marginBottom: 10 }}>{b.note}</div>
